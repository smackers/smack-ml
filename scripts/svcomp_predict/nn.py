...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, test_data=None):
        ...
        for j in range(epochs):
            ...
            if test_data:
                print("Epoch {0}: {1}".format(j, float(self.evaluate(test_data)*100.0 /n_test)))
            else:
...
if __name__ == '__main__':
    ...
    n = int(input("Enter '1' to restart (any other key to resume): "))
    #initializing network (# of neurons in each layer size(net) = #of layers)
    net = Network([33,25,11])
    net.SGD(training_data,500,20,0.1,test_data)
